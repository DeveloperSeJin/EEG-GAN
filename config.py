...
def get_config(case):
    if case == 'train_decoding': 
        ...
        parser.add_argument('-ne', '--num_epoch_step', type = int, help='num_epoch_step', default = 30, required=True)
        parser.add_argument('-lr', '--learning_rate_step', type = float, help='learning_rate_step', default = 0.0000005, required=True)
        parser.add_argument('-b', '--batch_size', type = int, help='batch_size', default = 32, required=True)
        ...
        parser.add_argument('--pre_trained_g', help='if you have a pre-trained EEG-To-Text model, enter the path of model', default = None)
        parser.add_argument('--pre_trained_d', help='if you have a pre-trained discriminator model, enter the path of model', default = None)
        parser.add_argument('--save_name', help='checkpoint save name', default = 'test', required=True)
